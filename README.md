...
- `VesselPhotoService` — a freely-licensed vessel photograph by IMO number, via two public Wikimedia APIs: Wikidata (the ship item carrying that IMO in `P458`, and its `P18` image) then Commons (the thumbnail at the requested width, plus the photographer and licence). `photo(imo:width:)` returns a `VesselPhoto` — image URL, Commons page, vessel name, `author`, `licence` and a ready-to-render `credit` — or `nil`, which is the common case: only vessels notable enough for Wikidata *and* pictured on Commons have one (large merchant and passenger ships often, pleasure craft essentially never). A miss is never an error. Lookups are cached in memory, misses included, so revisiting a target costs nothing. **The CC licences require attribution: show `credit` wherever the image is shown.**
...
**Curve histories** — `MetricHistories` smooths and retains the charted metrics (TWS, TWD, AWS, AWA, SOG, COG, depth, water temperature: 5 s over 1 h and 1 min over 6 h, angles by circular mean; pressure: 30 min over 48 h). `BoatMetricStore` keeps one, and the bridge opens others for hosts that merge their own connections (`boattools_bridge_history_open` / `_add` / `_series` / `_close`).

**Sun** — offline, no network, the same minutes on every platform.
- `SunTimes` — sunrise, solar noon and sunset of a local day at a position (NOAA solar calculator algorithm, about a minute's accuracy up to ±72° latitude); `Horizon` selects the official events or civil / nautical / astronomical dawn and dusk; `polar` reports a day when the Sun neither rises nor sets. `SunTimes.upcoming(latitude:longitude:after:)` gives the next sunrise, solar noon and sunset as `SunEvent`s, soonest first — what the apps' Sun pill shows. The bridge exposes them as `boattools_bridge_sun_times` and `boattools_bridge_sun_upcoming`.
