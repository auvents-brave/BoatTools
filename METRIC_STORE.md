...
- [ ] `BoatMetricStore` — `@Observable` class, multiplatform
  - `metrics: [String: BoatMetric]` — all numeric values
  - `aisTargets: [Int: AISTarget]` — AIS targets by MMSI
  - `satellites: [String: [SatelliteInfo]]` — satellite lists by constellation
  - History accessors
- [ ] `FrameCollector` — 1-second rolling buffer, fed by `NMEAFrame` stream
- [ ] `PriorityResolver` — applies tables above, produces `FlushResult`
- [ ] `RingBuffer<T>` — generic fixed-capacity circular buffer
- [ ] `TieredHistory` — two-tier (5 s + 1 min) history for wind/nav metrics
- [ ] `PressureHistory` — single-tier 30 min / 48 h ring buffer
- [ ] AIS staleness manager — marks/removes targets after 10/30 min
- [ ] AppGroup sink — Darwin-only JSON snapshot to shared defaults
- [ ] `NMEATransport` convenience — `func pipe(into store: BoatMetricStore) -> Task<Void, Error>`
