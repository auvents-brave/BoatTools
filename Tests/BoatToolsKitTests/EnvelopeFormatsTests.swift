...
struct EnvelopeFormatsTests {
	...
	@Test func `Canboat PLAIN line decodes to the right PGN, source and payload`() throws {
		let line =
			"2022-09-23T11:05:12.918Z,2,127489,236,255,26,"
			+ "00,28,00,ff,ff,bb,71,a7,03,00,00,e0,b0,05,00,ff,ff,ff,ff,ff,20,00,00,00,7e,ff"
		guard case .nmea2000(let pgn, let source, let priority, let data)? = CanboatPlainParser.parse(line) else {
	...
	@Test func `YD RAW with a timestamp and direction prefix decodes`() throws {
		...
		guard case .nmea2000(let pgn, let source, _, let data)? = YachtDevicesRawParser.parse(line) else {
			Issue.record("expected .nmea2000")
			return
		}
		#expect(pgn == 128267)
	...
	@Test func `Bare YD RAW without a prefix still decodes`() throws {
		...
		guard case .nmea2000(let pgn, let source, _, _)? = YachtDevicesRawParser.parse(line) else {
			Issue.record("expected .nmea2000")
			return
		}
		#expect(pgn == 128267)
	...
	@Test func `iKonvert PDGY line decodes its Base64 payload to the right PGN`() throws {
		...
		guard case .nmea2000(let pgn, let source, let priority, let data)? = IKonvertParser.parse(line) else {
			Issue.record("expected .nmea2000")
			return
		}
		#expect(pgn == 127250)
	...
	@Test func `iKonvert AIS PGN decodes to an AIS target`() throws {
		...
		guard case .nmea2000(let pgn, let source, _, let data)? = IKonvertParser.parse(line) else {
			Issue.record("expected .nmea2000")
			return
		}
		#expect(pgn == 129039)
