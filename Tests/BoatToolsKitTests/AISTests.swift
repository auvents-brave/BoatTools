...
struct AISTests {
  ...
  @Test func `Type 8 IFM 11 — meteorological metrics decode to wind, temperature, pressure`() throws
  {
    ...
    #expect(byName["TWS"] == 15)
    #expect(byName["TWS.gust"] == 22)
    #expect(byName["TWD"] == 270)
    #expect(byName["TWD.gust"] == 280)
    #expect(byName["temperature.air"].map { abs($0 - 21.5) < 1e-6 } ?? false)
  ...
  @Test func `Multi-part type 5 with a zero sequential ID assembles and decodes the ship name`()
    async throws
  {
