...
public struct NMEATransportConfig: Sendable {
	...
	/// Called on the transport's consumer task — the closure must be `@Sendable`.
	public var rawLogger: (@Sendable (String) -> Void)?

	/// Creates a transport configuration.
	public init(
		mode: NMEATransportMode,
...
/// TCP/UDP transport for NMEA 0183, NMEA 2000, and Signal K data, built on the
/// portable transport layer — ``NetworkStack`` (SwiftNIO on Apple/Linux, libcurl and
/// Winsock on Windows).
///
/// All mutable state (``LineAggregator``, ``FrameDispatcher``, the assemblers)
/// is confined to the single task that consumes the byte stream — no locks are
/// needed.
///
/// ## Usage
/// ```swift
...
public struct NMEATransport: Sendable {
	...
	public init(config: NMEATransportConfig) {
		self.config = config
	}

	/// Opens a connection and returns its frame stream.
	/// Pipe with ``BoatMetricStore/pipe(_:)``.
	public static func frameStream(config: NMEATransportConfig) -> AsyncThrowingStream<NMEAFrame, any Error> {
		NMEATransport(config: config).stream()
	}
	...
	/// the caller discards the `AsyncThrowingStream` (which closes the socket).
	public func stream() -> AsyncThrowingStream<NMEAFrame, any Error> {
		...
		return AsyncThrowingStream { continuation in
			let task = Task {
				...
				let dispatcher = FrameDispatcher(config: cfg) { continuation.yield($0) }
				do {
					let chunks: AsyncThrowingStream<[UInt8], any Error>
					switch cfg.mode {
					case .tcp(let host, let port):
						chunks = try await NetworkStack.tcp.connect(host: host, port: port).incoming
					case .udp(let port, let multicastGroup):
						chunks = try await NetworkStack.udp.listen(
							port: port, multicastGroup: multicastGroup)
					}
					// Dropping `chunks` on exit (return, throw or cancellation)
					// terminates the inner stream, which closes the socket.
...
///
/// Not thread-safe — confined to the transport's consumer task (same as
/// ``FrameDispatcher``).
private final class MultipartAssembler {
...
///
/// Not thread-safe — confined to the transport's consumer task alongside
/// ``FrameDispatcher``.
private final class AISTargetTracker {
...
/// frames reset the accumulator. Confined to the transport's consumer task
/// alongside ``FrameDispatcher`` — not thread-safe.
private final class FastPacketAssembler {
...
/// transport's consumer task alongside ``FrameDispatcher``.
private final class GSVAssembler {
...
///
/// Confined to the single task that consumes the byte stream — `ingest` calls
/// are never concurrent for the same instance. Shared with ``SignalKClient``'s
/// raw TCP path, which carries UTF-8 JSON rather than ASCII sentences.
final class LineAggregator {
	private let encoding: String.Encoding
	private var buffer: [UInt8] = []

	/// Creates an aggregator decoding lines with the given encoding.
	/// NMEA sentences are ASCII; Signal K NDJSON is UTF-8.
	...
	func ingest(_ bytes: [UInt8], emit: (String) -> Void) {
		...
		while let nl = buffer.firstIndex(of: 0x0A) {
			...
			if let line = String(bytes: trimmed, encoding: encoding)?
...
///
/// Also confined to the transport's consumer task.
private final class FrameDispatcher {
	private let config: NMEATransportConfig
				...
				case .nmea2000(let pgn, let src, let pri, let data) = frame
			{
				if NMEA2000Decoder.isFastPacket(pgn) {
					if let full = fastPacketAssembler.feed(pgn: pgn, source: src, frameData: data) {
						let assembled: NMEAFrame = .nmea2000(
							pgn: pgn, source: src,
...
private final class _Cell<T: Sendable>: @unchecked Sendable {
	var value: T
...
extension NMEATransport {
	...
	private static func nmeaRMCTimestamp(_ line: String) -> Date? {
		...
		let t = fields[1]
		let d = fields[9]  // HHMMSS.ss  DDMMYY
		guard t.count >= 6, d.count == 6 else { return nil }
		let hh = t.prefix(2)
		let mm = t.dropFirst(2).prefix(2)
		let ss = t.dropFirst(4).prefix(2)
		let dd = d.prefix(2)
		let mo = d.dropFirst(2).prefix(2)
		let yy = d.suffix(2)
