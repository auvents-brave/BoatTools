  ...
  // swift-nio (NIOPosix) provides the TCP/UDP stack but has no Windows build, so
  // the entire transport is compiled out there; Signal K / NMEA decoding, the
  // simulator and the metric store remain available.
  ...
  public struct NMEATransport: Sendable {
    ...
    public static func frameStream(config: NMEATransportConfig) -> AsyncThrowingStream<
      NMEAFrame, any Error
    > {
    ...
    public func stream() -> AsyncThrowingStream<NMEAFrame, any Error> {
      ...
      return AsyncThrowingStream { continuation in
        ...
        case .tcp(let host, let port):
          future = Self.bootstrapTCP(
            group: elg, host: host, port: port,
            config: cfg, emit: emit)
        case .udp(let port, let mc):
          future = Self.bootstrapUDP(
            group: elg, port: port,
            multicastGroup: mc,
            config: cfg, emit: emit)
        }
    ...
    private static func bootstrapTCP(
    ...
    private static func bootstrapUDP(
      group: any EventLoopGroup,
      port: Int,
  ...
  private final class MultipartAssembler: @unchecked Sendable {
    ...
    func feed(fields: [String]) -> (payload: String, fillBits: Int, channel: Character)? {
      ...
      let fillStr =
        fields[6].split(separator: "*", maxSplits: 1).first.map(String.init) ?? fields[6]
  ...
  private final class AISTargetTracker: @unchecked Sendable {
  ...
  private final class FastPacketAssembler: @unchecked Sendable {
  ...
  private final class GSVAssembler: @unchecked Sendable {
    ...
    func feed(fields: [String]) -> [NMEAFrame]? {
      ...
      if !snrs.isEmpty {
        ...
        out.append(
          .metric(.init(name: "\(prefix).snr.max", value: Double(snrs.max()!), unit: "dB")))
        out.append(
          .metric(.init(name: "\(prefix).snr.min", value: Double(snrs.min()!), unit: "dB")))
      }
      return out
    }
  }
  ...
  private final class LineAggregator: @unchecked Sendable {
  ...
  private final class FrameDispatcher: @unchecked Sendable {
    private let config: NMEATransportConfig
  ...
  private final class NMEAHandlerTCP: ChannelInboundHandler, @unchecked Sendable {
  ...
  private final class NMEAHandlerUDP: ChannelInboundHandler, @unchecked Sendable {
