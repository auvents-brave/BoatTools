  ...
  // The HTTP stack (AsyncHTTPClient) used here has no Windows build, so the whole
  // client is compiled out there; the `vrm` CLI command prints a "not yet
  // supported" notice instead.
  ...
  public final class VictronVRMClient: @unchecked Sendable {
    ...
    public static func installations(accessToken: String, userId: Int) async throws
      -> [Installation]
    {
    ...
    public func metrics(siteId: Int) async throws -> [BoatMetric] {
      ...
      return records.compactMap { r in
        ...
        if let canonical = Self.canonicalName(
          device: r.device, description: n, instance: r.instance)
        {
