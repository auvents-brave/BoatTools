...
public enum NMEASimulator {
		...
		AsyncThrowingStream { continuation in
			let task = Task {
				...
				while !Task.isCancelled {
					...
					emit(
						position: position, courseDegrees: course, speedKnots: speed,
						tick: tick, into: continuation)
					emitNavigation(
						position: position, target: target, course: course,
					...
					if stepMetres >= remaining {
						...
						if legIndex + 1 >= waypoints.count {
							guard loop else {
								emit(
									position: position, courseDegrees: course,
						...
						position = destination(
							from: position, distanceMetres: stepMetres,
	...
	public static func historyBackfill(
		...
		var tws: [HistorySample] = []
		var twd: [HistorySample] = []
		var aws: [HistorySample] = []
		var awa: [HistorySample] = []
		let windStep = 5.0
		for k in stride(from: 720, through: 0, by: -1) {
			...
			let apparent = apparentWind(
				trueSpeed: speed, trueDirection: direction,
	...
	private static func emit(
		...
		let apparent = apparentWind(
			trueSpeed: trueWindSpd, trueDirection: trueWindDir,
		...
		yieldFrame(
			pgn: 130306,
			data: windPayload(
				speedKnots: trueWindSpd,
				angleDegrees: trueWindDir, reference: 0),
			into: continuation)
		yieldFrame(
			pgn: 130306,
			data: windPayload(
				speedKnots: apparent.speed,
				angleDegrees: apparent.angle, reference: 2),
			into: continuation)
		yieldFrame(pgn: 130314, data: pressurePayload(hPa: pressureHPa(n)), into: continuation)
		yieldFrame(
			pgn: 127258, data: variationPayload(degrees: magneticVariationDegrees(at: position)),
			into: continuation)
		yieldFrame(
			pgn: 127488, data: engineRapidPayload(instance: 0, rpm: engineRPM(instance: 0, n)),
			into: continuation)
		yieldFrame(
			pgn: 127488, data: engineRapidPayload(instance: 1, rpm: engineRPM(instance: 1, n)),
			into: continuation)

		// Magnetic heading alongside the true one (compass = true − variation).
		let variation = magneticVariationDegrees(at: position)
		yieldFrame(
			pgn: 127250, data: headingPayload(degrees: heading - variation, magnetic: true),
			into: continuation)

		// Attitude — a gentle pitch & roll as the boat works in a seaway.
		yieldFrame(
			pgn: 127257,
			data: attitudePayload(
		...
		emitBattery(
			instance: 0, voltage: engineBatteryVoltage(n), current: 8 + 2 * sin(n * 0.05),
			soc: 100, into: continuation)
		emitBattery(
			instance: 1, voltage: serviceVoltage(n), current: -12 - 4 * sin(n * 0.03),
			soc: serviceSOC(n), into: continuation)
		emitBattery(
			instance: 2, voltage: 13.4 + 0.2 * sin(n * 0.02), current: solarCurrent(n),
			soc: solarSOC(n), into: continuation)

		// Temperatures: sea, outside air, refrigerator, freezer.
		yieldFrame(
			pgn: 130312,
			data: temperaturePayload(
				instance: 0, source: 0,
		...
		yieldFrame(
			pgn: 128275,
			data: distanceLogPayload(
				totalNM: 12_840 + n * 0.0017, tripNM: n * 0.0017), into: continuation)

		// Depth (over a shelving bottom), relative humidity, and wind gusts.
		yieldFrame(
			pgn: 128267, data: depthPayload(metres: 45 + 18 * sin(n * 0.004)),
			into: continuation)
		yieldFrame(
			pgn: 130311, data: humidityPayload(percent: 62 + 8 * sin(n * 0.0009)),
			into: continuation)
		yieldFrame(
			pgn: 130323, data: gustPayload(gustKnots: trueWindSpd + 3 + 2 * sin(n * 0.05)),
			into: continuation)
	}
	...
	private static func fluidLevelPayload(
		instance: UInt8, type: UInt8,
		level: Double, capacityL: Double
	) -> [UInt8] {
	...
	/// (129283) for the leg currently being sailed, plus the route plan with
	/// its waypoint names (129285) at a slow cadence, like real plotters do.
	private static func emitNavigation(
		position: GeoPoint, target: GeoPoint, course: Double, speedKnots: Double,
		remainingMetres: Double, route: SimulatorRoute, legIndex: Int, tick: Int,
		into continuation: AsyncThrowingStream<NMEAFrame, any Error>.Continuation
	) {
		yieldFrame(
			pgn: 129284,
			data: navigationDataPayload(
				target: target, distanceMetres: remainingMetres, bearing: course,
				speedKnots: speedKnots, destinationNumber: legIndex + 1), into: continuation)
		yieldFrame(
			pgn: 129283,
			data: crossTrackErrorPayload(
				metres: 40 * sin(Double(tick) * 0.03)), into: continuation)
		if tick % 30 == 0 {
			yieldFrame(pgn: 129285, data: routeInfoPayload(route: route), into: continuation)
		}
	}
	...
	private static func navigationDataPayload(
		...
		put(brg, at: 13)
		put(brg, at: 15)
		// Origin and destination waypoint numbers — indices into the 129285 plan.
		put(littleEndian(UInt32(max(0, destinationNumber - 1))), at: 17)
		put(littleEndian(UInt32(destinationNumber)), at: 21)
	...
	/// PGN 129285 — Navigation Route / WP Information: the route name and the
	/// numbered waypoint names ("WP 1", "WP 2", …) referenced by 129284.
	private static func routeInfoPayload(route: SimulatorRoute) -> [UInt8] {
		func varString(_ s: String) -> [UInt8] {
			let ascii = s.replacingOccurrences(of: "→", with: "-")
				.unicodeScalars.compactMap { $0.isASCII ? UInt8($0.value) : nil }
				.prefix(24)
			return [UInt8(ascii.count + 2), 1] + ascii
		}
		var d: [UInt8] = []
		d += littleEndian(UInt16(0))  // start RPS
		d += littleEndian(UInt16(route.waypoints.count))  // WPs in this message
		d += littleEndian(UInt16(1))  // database id
		d += littleEndian(UInt16(1))  // route id
		d.append(0)  // direction + flags
		d += varString(route.name)
		d.append(0xFF)  // reserved
		for (index, point) in route.waypoints.enumerated() {
			d += littleEndian(UInt16(index))
			d += varString("WP \(index + 1)")
			d += littleEndian(Int32((point.latitude * 1e7).rounded()))
			d += littleEndian(Int32((point.longitude * 1e7).rounded()))
		}
		return d
	}
	...
	private static let aisFleet: [AISShip] = [
		...
		AISShip(
			mmsi: 247_990_101, name: "SAN ROCCO", type: .fishing, classB: true,
			start: GeoPoint(latitude: 41.60, longitude: 9.30), course: 120, speed: 6.5),
		AISShip(
			mmsi: 228_990_111, name: "BONIFACIO", type: .highSpeedCraft, classB: false,
			start: GeoPoint(latitude: 41.38, longitude: 9.16), course: 300, speed: 28.0),
		AISShip(
			mmsi: 247_990_121, name: "MAGDALENA", type: .sailing, classB: true,
			start: GeoPoint(latitude: 41.30, longitude: 9.45), course: 280, speed: 5.5),
		AISShip(
			mmsi: 228_990_131, name: "VENT D'EST", type: .sailing, classB: true,
			start: GeoPoint(latitude: 42.30, longitude: 8.10), course: 160, speed: 6.0),
		AISShip(
			mmsi: 247_990_141, name: "TIRRENO", type: .cargo, classB: false,
			start: GeoPoint(latitude: 41.00, longitude: 9.80), course: 15, speed: 13.5),
		AISShip(
			mmsi: 256_990_151, name: "VALLETTA PRIDE", type: .cargo, classB: false,
		...
		AISShip(
			mmsi: 228_990_181, name: "PILOTE CORSE", type: .pilotVessel, classB: true,
	...
	private static func emitAISEvents(
		...
		// Safety broadcast — free-text safety message, like a coast station.
		if tick % 53 == 19 {
			continuation.yield(
				.aisTarget(
					AISTarget(
