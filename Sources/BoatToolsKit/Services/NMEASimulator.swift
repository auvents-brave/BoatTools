...
public enum NMEASimulator {
  ...
  private static func windPayload(speedKnots: Double, angleDegrees: Double, reference: UInt8)
    -> [UInt8]
  {
  ...
  private static func engineDynamicPayload(instance: UInt8, n: Double) -> [UInt8] {
    ...
    put(
      littleEndian(UInt16(bitPattern: Int16(((14.2 + 0.1 * sin(n * 0.05)) / 0.01).rounded()))),
      at: 7)  // alt voltage
  ...
  private static func gnssPositionPayload(_ p: GeoPoint, n: Double) -> [UInt8] {
    ...
    d[33] = 11  // satellites
    put(
      littleEndian(UInt16(bitPattern: Int16(((0.8 + 0.2 * sin(n * 0.02)) / 0.01).rounded()))),
      at: 34)  // HDOP
    put(
      littleEndian(UInt16(bitPattern: Int16(((1.5 + 0.3 * sin(n * 0.015)) / 0.01).rounded()))),
      at: 36)  // PDOP
  ...
  private static func gnssDopsPayload(n: Double) -> [UInt8] {
    func dop(_ v: Double) -> [UInt8] {
      littleEndian(UInt16(bitPattern: Int16((v / 0.01).rounded())))
    }
