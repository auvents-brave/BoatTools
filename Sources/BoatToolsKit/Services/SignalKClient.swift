  ...
  // swift-nio (NIOPosix) and the HTTP stack (AsyncHTTPClient / WebSocketKit) used
  // here have no Windows build, so the whole client is compiled out there; the
  // Signal K / NMEA decoders it exposes as static helpers live elsewhere and
  // remain available.
  ...
  public final class SignalKClient: @unchecked Sendable {
    ...
    public struct Config: Sendable {
      ...
      public static func liveStream(config: Config) async -> AsyncThrowingStream<
        NMEAFrame, any Error
      > {
        let client = SignalKClient(
          config: config, eventLoopGroup: MultiThreadedEventLoopGroup.singleton)
  ...
  private final class SignalKHandlerTCP: ChannelInboundHandler, @unchecked Sendable {
  ...
  private final class SignalKHandlerUDP: ChannelInboundHandler, @unchecked Sendable {
