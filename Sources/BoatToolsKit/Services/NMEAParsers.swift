...
public enum NMEA0183Parser {
  ...
  private static func decodeRMC(_ f: [String]) -> [BoatMetric]? {
    ...
    var out: [BoatMetric] = []
    if let lat = nmeaCoord(f[3], hemi: f[4]) {
      out.append(.init(name: "lat", value: lat, unit: "°"))
    }
    if let lon = nmeaCoord(f[5], hemi: f[6]) {
      out.append(.init(name: "lon", value: lon, unit: "°"))
    }
  ...
  private static func decodeMWV(_ f: [String]) -> [BoatMetric]? {
    ...
    if let s = Double(f[3]) {
      out.append(.init(name: isTrue ? "TWS" : "AWS", value: s, unit: f[4]))
    }
    return out.isEmpty ? nil : out
  }
  ...
  private static func decodeVBW(_ f: [String]) -> [BoatMetric]? {
    ...
    if f.count >= 4, f[3].hasPrefix("A") {
      ...
      if let t = Double(f[2]) {
        out.append(.init(name: "speed.water.transverse", value: t, unit: "kn"))
      }
    }
    if f.count >= 7, f[6].hasPrefix("A") {
      if let l = Double(f[4]) { out.append(.init(name: "SOG", value: l, unit: "kn")) }
      if let t = Double(f[5]) {
        out.append(.init(name: "speed.ground.transverse", value: t, unit: "kn"))
      }
    }
    return out.isEmpty ? nil : out
  }
  ...
  private static func decodeVHW(_ f: [String]) -> [BoatMetric]? {
    var out: [BoatMetric] = []
    if f.count >= 2, let ht = Double(f[1]) {
      out.append(.init(name: "HDG.true", value: ht, unit: "°"))
    }
    if f.count >= 4, let hm = Double(f[3]) {
      out.append(.init(name: "HDG.magnetic", value: hm, unit: "°"))
    }
  ...
  private static func decodeGLL(_ f: [String]) -> [BoatMetric]? {
    ...
    var out: [BoatMetric] = []
    if let lat = nmeaCoord(f[1], hemi: f[2]) {
      out.append(.init(name: "lat", value: lat, unit: "°"))
    }
    if let lon = nmeaCoord(f[3], hemi: f[4]) {
      out.append(.init(name: "lon", value: lon, unit: "°"))
    }
    return out.isEmpty ? nil : out
  }
  ...
  private static func decodeGGA(_ f: [String]) -> [BoatMetric]? {
    ...
    var out: [BoatMetric] = []
    if let lat = nmeaCoord(f[2], hemi: f[3]) {
      out.append(.init(name: "lat", value: lat, unit: "°"))
    }
    if let lon = nmeaCoord(f[4], hemi: f[5]) {
      out.append(.init(name: "lon", value: lon, unit: "°"))
    }
  ...
  private static func decodeMDA(_ f: [String]) -> [BoatMetric]? {
    ...
    if let d = Double(f[11]) {
      out.append(.init(name: "temperature.dewPoint", value: d, unit: "°C"))
    }
  ...
  private static func decodeRPM(_ f: [String]) -> [BoatMetric]? {
    ...
    var out: [BoatMetric] = []
    if let rpm = Double(f[3]) {
      out.append(.init(name: "\(bucket).\(inst).rpm", value: rpm, unit: "rpm"))
    }
    if let pitch = Double(f[4]) {
      out.append(.init(name: "\(bucket).\(inst).pitch", value: pitch, unit: "%"))
    }
    return out.isEmpty ? nil : out
  }
  ...
  private static func decodeBWC(_ f: [String]) -> [BoatMetric]? {
    ...
    if let b = Double(f[8]) {
      out.append(.init(name: "navigation.bearingToDest.magnetic", value: b, unit: "°"))
    }
  ...
  private static func decodeBWW(_ f: [String]) -> [BoatMetric]? {
    ...
    var out: [BoatMetric] = []
    if let b = Double(f[1]) {
      out.append(.init(name: "navigation.bearingNextLeg", value: b, unit: "°"))
    }
    if let b = Double(f[3]) {
      out.append(.init(name: "navigation.bearingNextLeg.magnetic", value: b, unit: "°"))
    }
    return out.isEmpty ? nil : out
  }
  ...
  private static func decodeRSD(_ f: [String]) -> [BoatMetric]? {
    ...
    if let b = Double(f[10]) {
      out.append(.init(name: "radar.cursor.bearing", value: b, unit: "°"))
    }
    return out.isEmpty ? nil : out
  }
  ...
  private static func decodeTNL_GGK(_ f: [String]) -> [BoatMetric]? {
    ...
    var out: [BoatMetric] = []
    if let lat = nmeaCoord(f[4], hemi: f[5]) {
      out.append(.init(name: "lat", value: lat, unit: "°"))
    }
    if let lon = nmeaCoord(f[6], hemi: f[7]) {
      out.append(.init(name: "lon", value: lon, unit: "°"))
    }
...
internal enum NMEA2000Decoder {
  ...
  private static func i16(_ d: [UInt8], _ at: Int) -> Int16? {
    u16(d, at).map { Int16(bitPattern: $0) }
  }
  ...
  private static func i32(_ d: [UInt8], _ at: Int) -> Int32? {
    u32(d, at).map { Int32(bitPattern: $0) }
  }
  ...
  private static func i64(_ d: [UInt8], _ at: Int) -> Int64? {
    u64(d, at).map { Int64(bitPattern: $0) }
  }
  ...
  private static func heading(_ d: [UInt8]) -> [BoatMetric]? {
    ...
    if let h = u16(d, 1), !na(h) {
      ...
      let name: String
      if let r = u8(d, 7) {
        name = (r & 0x03) == 0 ? "HDG.true" : "HDG.magnetic"
      } else {
        name = "HDG"
      }
    ...
    if let varia = i16(d, 5), !na(varia) {
      out.append(
        .init(name: "magneticVariation", value: Double(varia) * 1e-4 * 180 / .pi, unit: "°"))
    }
    return out.isEmpty ? nil : out
  }
  ...
  private static func attitude(_ d: [UInt8]) -> [BoatMetric]? {
    var out: [BoatMetric] = []
    if let yaw = i16(d, 1) {
      out.append(.init(name: "yaw", value: Double(yaw) * 1e-4 * 180 / .pi, unit: "°"))
    }
    if let pitch = i16(d, 3) {
      out.append(.init(name: "pitch", value: Double(pitch) * 1e-4 * 180 / .pi, unit: "°"))
    }
    if let roll = i16(d, 5) {
      out.append(.init(name: "roll", value: Double(roll) * 1e-4 * 180 / .pi, unit: "°"))
    }
    return out.isEmpty ? nil : out
  }
