...
internal enum AISDecoder {
	...
	private static func decodeMeteoIFM11(buf: AISBitBuffer) -> [BoatMetric]? {
		...
		// Everything here describes a *remote* meteorological/coast station — its
		// own position and the conditions measured at that station — so the metrics
		// live in the `meteo.` namespace. Sharing the generic names (`lat`, `lon`,
		// `TWS`, `temperature.air`, …) would let a weather broadcast overwrite the
		// own-ship GPS fix, wind and barometer in the shared metric store.
		let mmsi = buf.uint(8, 30)
		var out: [BoatMetric] = [.init(name: "meteo.mmsi", value: Double(mmsi))]
		...
		if lonRaw != 10_860_000 { out.append(.init(name: "meteo.lon", value: Double(lonRaw) / 60_000.0, unit: "°")) }
		if latRaw != 5_460_000 { out.append(.init(name: "meteo.lat", value: Double(latRaw) / 60_000.0, unit: "°")) }
		...
		if avgWind < 127 { out.append(.init(name: "meteo.TWS", value: Double(avgWind), unit: "kn")) }
		if gustWind < 127 { out.append(.init(name: "meteo.TWS.gust", value: Double(gustWind), unit: "kn")) }
		if windDir < 360 { out.append(.init(name: "meteo.TWD", value: Double(windDir), unit: "°")) }
		if gustDir < 360 { out.append(.init(name: "meteo.TWD.gust", value: Double(gustDir), unit: "°")) }

		// Temperature
		let tempRaw = buf.int(151, 11)  // signed
		if tempRaw > -601, tempRaw < 601 {  // valid: -60.0..60.0 °C
			out.append(.init(name: "meteo.temperature.air", value: Double(tempRaw) * 0.1, unit: "°C"))
		}

		// Humidity
		let humidity = buf.uint(162, 7)
		if humidity <= 100 {
			out.append(.init(name: "meteo.humidity", value: Double(humidity), unit: "%"))
		}

		// Dew point
		let dewRaw = buf.int(169, 10)
		if dewRaw > -201, dewRaw < 501 {  // valid: -20.0..50.0 °C
			out.append(.init(name: "meteo.temperature.dewPoint", value: Double(dewRaw) * 0.1, unit: "°C"))
		}

		// Air pressure — encoded with offset 800.
		let pressRaw = buf.uint(179, 9)
		if pressRaw != 511, pressRaw <= 402 {
			out.append(.init(name: "meteo.pressure.atmospheric", value: Double(pressRaw + 800), unit: "hPa"))
		}
	...
	private static func decodeMeteoIFM31(buf: AISBitBuffer) -> [BoatMetric]? {
		...
		// See ``decodeMeteoIFM11`` — these are a remote station's readings, kept in
		// the `meteo.` namespace so they never overwrite own-ship metrics.
		let mmsi = buf.uint(8, 30)
		var out: [BoatMetric] = [.init(name: "meteo.mmsi", value: Double(mmsi))]
		...
		if pressRaw > 0, pressRaw <= 402 {
			out.append(.init(name: "meteo.pressure.atmospheric", value: Double(pressRaw + 799), unit: "hPa"))
		}
	...
	private static func decodeN2KPosA(_ d: [UInt8]) -> AISTarget? {
		...
		return AISTarget(
			...
			rateOfTurn: rotRaw != Int16.max ? Int((Double(rotRaw) * 3.125e-5 * 180 / .pi * 60).rounded()) : nil,
			positionAccuracy: posAcc, raim: raim,
