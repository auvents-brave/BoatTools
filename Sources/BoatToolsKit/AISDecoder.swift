...
public struct AISTarget: Sendable, Equatable {
    ...
    /// Creates an AISTarget (memberwise).
    public init(
...
fileprivate struct AISBitBuffer {
...
public enum AISDecoder {
    ...
    public static func decode(
        ...
        guard buf.bitCount >= 6,
              let messageType = AisMessageType(rawValue: buf.uint(0, 6)) else { return nil }
    ...
    public static func decodeMeteoMetrics(payload: String, fillBits: Int) -> [BoatMetric]? {
        let buf = AISBitBuffer(payload, fillBits: fillBits)
    ...
    public static func decodeN2K(pgn: UInt32, source: UInt8, data: [UInt8]) -> AISTarget? {
        switch pgn {
    ...
    private static func n2kU32(_ d: [UInt8], _ at: Int) -> UInt32 {
        guard at + 3 < d.count else { return 0xFFFFFFFF }
