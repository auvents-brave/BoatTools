...
public enum NMEA0183Parser {
    ...
    private static func decodeZDA(_ f: [String]) -> [BoatMetric]? {
        ...
        components.year = yr; components.month = mo; components.day = d
    ...
    private static func decodeXTE(_ f: [String]) -> [BoatMetric]? {
        guard f.count >= 6, f[1].hasPrefix("A"),
              let mag = Double(f[3]) else { return nil }
    ...
    private static func decodeBOD(_ f: [String]) -> [BoatMetric]? {
        ...
        if let b = Double(f[3]) { out.append(.init(name: "navigation.bearingOriginToDest.magnetic", value: b, unit: "°")) }
        return out.isEmpty ? nil : out
    }
...
public enum YachtDevicesRawParser {
    ...
    public static func parse(_ line: String) -> NMEAFrame? {
...
public enum SeaSmartParser {
    ...
    public static func parse(_ line: String) -> NMEAFrame? {
        ...
        guard fields.count >= 5,
              ...
              let src = UInt8(fields[3], radix: 16) else { return nil }
        let hex = fields[4]
        var data: [UInt8] = []
...
public enum CanboatPlainParser {
...
public enum NMEA2000Decoder {
    ...
    ///
    /// NOTE: PGNs > 8 bytes are fast-packet; the data payload must already be
    /// reassembled before being passed here. SeaSmart `$PCDIN` carries reassembled
    /// payloads natively; raw YD format does not (single CAN frame only).
    ///
    ...
    ///
    /// Used by ``FrameDispatcher`` to route raw YD/RAW CAN frames through a
    /// fast-packet assembler. SeaSmart `$PCDIN` already carries reassembled
    /// payloads so the dispatcher bypasses the assembler in that path.
    public static func isFastPacket(_ pgn: UInt32) -> Bool {
    ...
    public static func isAISPGN(_ pgn: UInt32) -> Bool {
    ...
    public static func decodeFrames(pgn: UInt32, data: [UInt8]) -> [NMEAFrame]? {
        switch pgn {
    ...
    public static func decode(pgn: UInt32, data: [UInt8]) -> [BoatMetric]? {
        switch pgn {
    ...
    private static func na(_ v: UInt32) -> Bool { v == 0xFFFFFFFF }
    private static func na(_ v: UInt64) -> Bool { v == 0xFFFFFFFFFFFFFFFF }
    ...
    private static func heading(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let h = u16(d, 1), !na(h) {
            ...
            let name: String
            if let r = u8(d, 7) { name = (r & 0x03) == 0 ? "HDG.true" : "HDG.magnetic" }
            else { name = "HDG" }
    ...
    private static func attitude(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let pitch = i16(d, 3) { out.append(.init(name: "pitch", value: Double(pitch) * 1e-4 * 180 / .pi, unit: "°")) }
    ...
    private static func engineRapid(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let boost = u16(d, 3), !na(boost) {
            out.append(.init(name: "engine.\(inst).boostPressure",
                             value: Double(boost) * 100, unit: "Pa"))
        }
        if let raw = u8(d, 5), raw != 0x7F {
            out.append(.init(name: "engine.\(inst).tiltTrim",
    ...
    private static func fluidLevel(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let level = i16(d, 1), level != Int16.max {
            out.append(.init(name: "\(bucket).\(instance).level",
                             value: Double(level) * 0.004, unit: "%"))
        }
        if let cap = u32(d, 3), !na(cap) {
            out.append(.init(name: "\(bucket).\(instance).capacity",
    ...
    private static func windlassControl(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let b1 = u8(d, 1) {
            ...
            out.append(.init(name: "windlass.\(i).commandedDirection",
    ...
    private static func windlassOperating(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let raw = u16(d, 1), !na(raw) {
            out.append(.init(name: "windlass.\(i).chainLength",
                             value: Double(raw) * 0.1, unit: "m"))
        }

        // Chain speed
        if let spd = u8(d, 3), spd != 0xFF {
            out.append(.init(name: "windlass.\(i).chainSpeed",
        ...
        if let b4 = u8(d, 4) {
            ...
            if motion != 0 {
                ...
                out.append(.init(name: "windlass.\(i).motion",
            ...
            if docking == 1 || docking == 2 {
                out.append(.init(name: "windlass.\(i).anchorUp",
            ...
            let rode = (b4 >> 2) & 0x03
            if rode == 1 || rode == 2 {
                out.append(.init(name: "windlass.\(i).rodeType",
    ...
    private static func windlassMonitoring(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let hrs = u8(d, 1), hrs != 0xFF {
            out.append(.init(name: "windlass.\(i).motorHours",
                             value: Double(hrs), unit: "h"))
        }
        if let v = u16(d, 2), !na(v) {
            out.append(.init(name: "windlass.\(i).controllerVoltage",
                             value: Double(v) * 0.01, unit: "V"))
        }
        if let a = u16(d, 4), !na(a) {
            out.append(.init(name: "windlass.\(i).motorCurrent",
    ...
    private static func engineDynamic(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let t = u16(d, 3), !na(t) {
            out.append(.init(name: "engine.\(inst).oilTemperature",
                             value: Double(t) * 0.1 - 273.15, unit: "°C"))
        }
        if let c = u16(d, 5), !na(c) {
            out.append(.init(name: "engine.\(inst).coolantTemperature",
                             value: Double(c) * 0.01 - 273.15, unit: "°C"))
        }
        if let v = i16(d, 7), !na(v) {
            out.append(.init(name: "engine.\(inst).alternatorVoltage",
                             value: Double(v) * 0.01, unit: "V"))
        }
        ...
        if let raw = u8(d, 24), raw != 0x7F {
            out.append(.init(name: "engine.\(inst).load",
                             value: Double(Int8(bitPattern: raw)), unit: "%"))
        }
        if let raw = u8(d, 25), raw != 0x7F {
            out.append(.init(name: "engine.\(inst).torque",
                             value: Double(Int8(bitPattern: raw)), unit: "%"))
        }
        return out.isEmpty ? nil : out
    }
    ...
    private static func gnssPosition(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let geoid = i32(d, 38), !na(geoid) {
            out.append(.init(name: "gps.geoidalSeparation",
    ...
    private static func timeAndDate(_ d: [UInt8]) -> [BoatMetric]? {
        guard let date = u16(d, 0), !na(date),
              let time = u32(d, 2), !na(time) else { return nil }
        let epoch = Double(date) * 86_400.0 + Double(time) / 10_000.0
    ...
    private static func temperature(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let setRaw = u16(d, 5), !na(setRaw) {
            out.append(.init(name: "\(name).setpoint",
    ...
    private static func envWithHumidity(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let t = u16(d, 2), !na(t) {
            out.append(.init(name: temperatureSourceName(tempSource),
                             value: Double(t) * 0.01 - 273.15, unit: "°C"))
        }
    ...
    private static func navigationData(_ d: [UInt8]) -> [BoatMetric]? {
        ...
        if let b = u16(d, 15), !na(b) {
            out.append(.init(name: "navigation.bearingToDest",
                             value: Double(b) * 1e-4 * 180 / .pi, unit: "°"))
        }
        ...
        if let vmg = i16(d, 33), !na(vmg) {
            out.append(.init(name: "navigation.vmg",
