...
fileprivate func formatCoordDDM(_ dd: Double, isLatitude: Bool) -> String {
    let abs = Swift.abs(dd)
    let deg = Int(abs)
    let min = (abs - Double(deg)) * 60.0
    let hemi: String
    if isLatitude  { hemi = dd >= 0 ? "N" : "S" }
    else           { hemi = dd >= 0 ? "E" : "W" }
...
fileprivate func formatMetricValue(_ m: BoatMetric) -> String {
    let v = m.value
    switch m.name {
    case "lat",
         "waypoint.lat",
         "weatherStation.lat":   return formatCoordDDM(v, isLatitude: true)
    case "lon",
         "waypoint.lon",
    ...
    case "temperature.water",
         "temperature.air":      return String(format: "%.1f °C", v)
    ...
    case "gps.pdop",
         "gps.hdop",
         "gps.vdop",
         "gps.tdop":             return String(format: "%.1f", v)
    case "gps.quality":
        switch Int(v) {
    ...
    case "gps.rms",
         "gps.error.lat",
         "gps.error.lon",
         "gps.error.alt":        return String(format: "%.2f m", v)
    case "tilt":                 return String(format: "%.2f°", v)
    case "pjk.northing",
         "pjk.easting":          return String(format: "%.3f m", v)
    ...
    case "navigation.bearingOriginToDest",
         ...
         "navigation.headingToSteer": return String(format: "%.1f°", v)
    ...
    case "radar.rangeScale",
         "radar.cursor.range":   return String(format: "%.2f NM", v)
    case "radar.cursor.bearing": return String(format: "%.1f°", v)
    case "route.id",
         "route.waypointsInMessage": return String(format: "%.0f", v)
    ...
    case "HDG.deviation",
         "magneticVariation":    return String(format: "%+.2f°", v)
    ...
    case "COG.magnetic",
         "TWD.magnetic":         return String(format: "%.1f° M", v)
    case "dsc.format",
         "dsc.category":         return String(format: "%.0f", v)
    case "dsc.mmsi",
         "dse.mmsi":             return String(format: "%.0f", v)       // MMSI is a 9–10 digit int
    default:
        ...
        if m.name.hasSuffix(".snr.avg")
            ...
            || m.name.hasSuffix(".snr.min") { return String(format: "%.0f dB", v) }
        if m.name.hasSuffix(".hdop")
            || m.name.hasSuffix(".pdop")
            || m.name.hasSuffix(".vdop")    { return String(format: "%.1f", v) }
...
fileprivate let n2kPGNNames: [(pgn: UInt32, name: String)] = [
    (126992, "System Time"),
    (127245, "Rudder"),
    (127250, "Heading"),
    (127251, "Rate of Turn"),
...
fileprivate func render(_ frame: NMEAFrame) {
    switch frame {
...
fileprivate func renderVER(fields: [String]) {
...
fileprivate func renderAIS(_ t: AISTarget) {
...
/// would block forever.
fileprivate func consumeFrames(
...
fileprivate func renderVRMDiagnostics(_ records: [VictronVRMClient.DiagnosticRecord], siteId: Int) {
...
/// access to the single NIO event-loop thread, so the unchecked conformance is
/// safe and keeps the capture portable to Linux.
fileprivate final class UncheckedSendableBox<Value>: @unchecked Sendable {
    let value: Value
...
/// `Sendable`-safe inside the NIO event-loop logger closure.
fileprivate func ydRawCaptureTimestamp() -> String {
...
struct ConnectCommand: AsyncParsableCommand {
    ...
    @Option(name: .shortAndLong,
            help: ArgumentHelp("REST snapshot poll interval in seconds (0 = one-shot; ignored for live streams)", valueName: "sec"))
    var watch: Int = 0
    ...
    func run() async throws {
        ...
        do {
            ...
            case .tcp(let h, let p):
                if format == .signalk {
                    try await consumeFrames(
                        SignalKClient.tcpStream(host: h, port: p, on: elg, rawLogger: logger),
                        duration: duration)
                } else {
                    let cfg = NMEATransportConfig(
                        ...
                        rawLogger: logger)
                    try await consumeFrames(
                        NMEATransport(config: cfg, group: elg).stream(),
                        duration: duration)
                }
            ...
            case .udpBroadcast(let p):
                if format == .signalk {
                    try await consumeFrames(
                        SignalKClient.udpStream(listenPort: p, on: elg, rawLogger: logger),
                        duration: duration)
                } else {
                    let cfg = NMEATransportConfig(
            ...
            case .web(let urlStr):
                try await runWeb(urlStr: urlStr, elg: elg, rawLogger: logger)
            }
    ...
    /// and all writes come from the single NIO event-loop thread, so no lock needed.
    ///
    ...
    private func runWeb(urlStr: String, elg: any EventLoopGroup,
        ...
        do {
            if wantsLive {
                let stream = await client.liveStream(on: elg, rawLogger: rawLogger)
    ...
    private func runRealtime(_ stream: AsyncThrowingStream<FileFrame, any Error>) async throws {
        ...
        for try await ff in stream {
            if let ts = ff.timestamp {
                let now = Date()
                if wallAnchor == nil { wallAnchor = now; logAnchor = ts }
