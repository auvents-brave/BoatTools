...
private let ANSI_RESET: String = isStdoutTTY ? "\u{1B}[0m" : ""
  ...
  private func reportWebUnsupportedOnWindows(_ feature: String) throws {
    let message =
      "\(feature) is not yet supported on Windows. Use a tcp:// or udp:// connection instead.\n"
...
private func render(_ frame: NMEAFrame) {
  ...
  case .gsvReport(let constellation, let inView, let satellites):
    ...
    for sat in satellites {
      ...
      print(
        "   \(ANSI_DIM)     PRN \(String(format: "%3d", sat.prn))  el \(el)  az \(az)  \(snr)\(ANSI_RESET)"
      )
    }
...
private func renderAIS(_ t: AISTarget) {
  ...
  if let lat = t.latitude, let lon = t.longitude {
    print(
      "   ↳ position      \(formatCoordDDM(lat, isLatitude: true))  \(formatCoordDDM(lon, isLatitude: false))"
    )
  }
...
extension String {
  ...
  private func renderVRMDiagnostics(_ records: [VictronVRMClient.DiagnosticRecord], siteId: Int) {
    ...
    print(
      "— Site \(siteId) diagnostics, \(records.count) records across \(groups.count) device(s) —")
...
// SignalKClient), which does not build on Windows; they are compiled out there.
  ...
  struct ConnectCommand: AsyncParsableCommand {
    ...
    @Option(
      name: .shortAndLong,
      help:
        "Port — alone: UDP broadcast; +--host: TCP; +--multicast: UDP multicast. Incompatible with --url."
    )
    var port: Int?
    ...
    @Option(
      name: .shortAndLong,
      help: ArgumentHelp(
        "REST snapshot poll interval in seconds (0 = one-shot; ignored for live streams)",
        valueName: "sec"))
    var watch: Int = 0
    ...
    fileprivate func resolveTransport() throws -> Transport {
      ...
      guard hasURL || port != nil else {
        throw ValidationError(
          "Specify either --url or --port (with optional --host or --multicast)")
      }
    ...
    private func runWeb(
      urlStr: String, elg: any EventLoopGroup,
  ...
  struct FileCommand: AsyncParsableCommand {
    ...
    @Option(
      help:
        "Wire format (auto|nmea0183|ydraw|seasmart|signalk|canboat|ikonvert; default: auto-detect)")
    var format: WireFormat = .auto
    ...
    private func runAtRate(_ stream: AsyncThrowingStream<FileFrame, any Error>, fps: Double)
      async throws
    {
...
// compiled out on Windows; the command is therefore unavailable there too.
  ...
  struct DiscoverCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
      ...
      abstract:
        "Browse mDNS servers on the LAN (Signal K + NMEA 0183 TCP), then connect interactively."
    )
